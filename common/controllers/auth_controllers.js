...
const randomTokenGen = require('../helpers/generate_token');
const passwordEncrypt = require('../helpers/password_encrypt');
const { getUser } = require('../../user_service/user');
const { getToken } = require('../../token_service/token');
...
const registerUser = async (req, res) => {
  ...
  try {
    ...
    if (emailExist) {
      return res.status(409).json({ error_msg: 'E-Mail already exists' });
    }
...
const loginUser = async (req, res) => {
  ...
  try {
    ...
    if (!validPass) {
      return res.status(403).json({ error_msg: 'Invalid password' });
    }
...
const verifyUserRegistration = async (req, res) => {
  ...
  try {
    ...
    if (user.status) {
      return res.status(409).json({ error_msg: 'User already verified' });
    }

    // This should not even happen. I am checking if the user email matches the user id in the token
    if (!(token._userId !== user._id)) {
      return res.status(401).json({ error_msg: 'Token does not match user' });
    }
...
const changePassword = async (req, res) => {
  try {
    ...
    if (admin) {
      ...
      if (!validPass) {
        return res.status(404).json({ error_msg: 'Current password is wrong' });
      }
      user.password = await passwordEncrypt(newPassword);
    }
    // Ensure new password not equals to old password
